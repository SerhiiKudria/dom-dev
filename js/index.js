...
const usersListItems = news.map((u) => createNewsListItem(u));

const newsListEl = document.querySelector(".newsList");
newsListEl.append(...usersListItems);
...
function createNewsListItem({ title, headerBgSrc, category, body, date }) {
  ...
  newsListItemEl.append(
    createPhoto(headerBgSrc, title),
    createNewsTitle(title),
...
function createNewsTitle(content) {
  ...
  userNameEl.classList.add("newsTitle");
  userNameEl.textContent = content;
  return userNameEl;
}

function createNewsCategory(content) {
  const userNameEl = document.createElement("h2");
  userNameEl.classList.add("newsCategory");
  userNameEl.textContent = content;
  return userNameEl;
}
...
function createDate(date) {
  const dateEI = document.createElement("a");
  dateEI.textContent = date;
  return dateEI;
}
